...
const payloadCache = new Map<PayloadSize, { json: string; compressed: Buffer }>()

async function initPayloads(): Promise<void> {
  for (const [size, targetBytes] of Object.entries(PAYLOAD_TARGET_BYTES) as [PayloadSize, number][]) {
    ...
    console.log(
      `[payload] ${size}: uncompressed=${json.length} bytes, compressed=${compressed.length} bytes`
    )
  }
}
...
function clientAcceptsGzip(acceptEncoding: string | null | undefined): boolean {
...
function parseQValue(qPart: string | undefined): number {
  if (!qPart) return 1
