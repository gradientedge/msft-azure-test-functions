...
// ---------------------------------------------------------------------------
// Payload generation — same data as the direct endpoint
// ---------------------------------------------------------------------------
...
function generatePayload(targetBytes: number): unknown[] {
...
const payloadCache = new Map<PayloadSize, unknown[]>()
...
fastifyApp.get<{ Params: { size: string } }>('/api/fastify-payload/:size', async (request, reply) => {
  ...
  const items = payloadCache.get(size as PayloadSize)!
...
fastifyApp.get<{ Params: { size: string } }>('/api/fastify-payload-no-gzip/:size', async (request, reply) => {
  const { size } = request.params
