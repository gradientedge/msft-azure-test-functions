...
const compressPlugin: FastifyPluginCallback<{ threshold?: number }> = (app, opts, done) => {
...
function parseEncodingQValue(qPart: string | undefined): number {
  if (!qPart) {
    return 1
  }
...
function mergeVary(reply: FastifyReply, field: string): void {
...
function getByteLength(payload: unknown): number | null {
